...
$time = $_POST['time'];
	...
	if($conn->connect_error){
		...
		$stmt = $conn->prepare("insert into nphhs(rice,dhaal,sugar,oil,wheat,kerosene,date,time) values(?,?,?,?,?,?, ?,?)");
		$stmt->bind_param("iiiiiiss", $rice,$dhaal,$sugar,$oil,$wheat,$kerosene,$date,$time);
		$execval = $stmt->execute();
		
		echo '<script>alert("Registration successfully...")</script>';
$sql = "SELECT id,rice,dhaal,sugar,oil,wheat,kerosene,date,time FROM nphhs ORDER BY id DESC LIMIT 1";
